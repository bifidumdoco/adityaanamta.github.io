...
class Game {
    ...
    async loadAssets() {
        ...
        const models = ['bismarck.glb', 'graf_spee_low_poly.glb', 'hms_ardrossan.glb', 'hms_aurora.glb', 'uss_nc_battleship.glb'];
            ...
            return new Promise((resolve, reject) => {
                loader.load(name, (gltf) => {
                    ...
                    if (xhr.total > 0) {
                        ...
                        // We will update the bar in the main Promise.all wrapper if needed, 
                        // but for simplicity let's just count loaded files
                    }
                }, (error) => {
        ...
        let loadedCount = 0;
        const promises = models.map(name => loadModel(name).then(() => {
            loadedCount++;
        ...
        // --- Load Audio ---
        // Listener attach moved to startGame
        ...
        // Load sounds but don't block start if they fail
        ...
        audioLoader.load('shipsenginesound/big-ship-stationary-1-26891.mp3', (buffer) => {
            ...
            this.engineSound.setVolume(0.2); // Low ambient
        });
    ...
    showShipSelection() {
        ...
        const models = Object.keys(this.assets);
        models.forEach(name => {
